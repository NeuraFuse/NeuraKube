...
import (
	...
	"../../../../../tools-go/vars"
	"../../../../../tools-go/users"
	//"../../../../../tools-go/kubernetes/deployments/templates"
...
func (f F) SetConfigs() {
	if config.ValidSettings("infrastructure", vars.InfraProviderGcloud, true) {
		logging.Log([]string{"", vars.EmojiSettings, vars.EmojiInfra}, "Using infrastructure provider: gcloud\n", 0)
	} else {
		logging.Log([]string{"\n", vars.EmojiInfra, vars.EmojiWarning}, "Invalid and/or missing gcloud settings in infrastructure config.\nStarting assistant to retrieve missing settings..\n", 0)
		infraconfig.F.SetAuth(infraconfig.F{})
		infraconfig.F.SetProject(infraconfig.F{})
		//infraconfig.F.SetCluster(infraconfig.F{})
		infraconfig.F.SetMachineSpec(infraconfig.F{})
	}
	vars.InfraProviderActive = vars.InfraProviderGcloud
}

func (f F) ClusterConfig(machineType, accType string) *containerpb.Cluster {
	cluster := &containerpb.Cluster{}
	cluster.Name = config.Setting("get", "infrastructure", "Spec.Cluster.Name", "")
	cluster.Description = "Created by " + vars.OrganizationName + " from user " + users.GetIDActive() + "."
	cluster.NodePools = f.NodePoolConfig(vars.OrganizationNameRepo, machineType, accType)
	cluster.MasterAuth = f.MasterAuth()
...
func (f F) MasterAuth() *containerpb.MasterAuth {
	masterAuth := &containerpb.MasterAuth{}
	masterAuth.Username = "admin"
	masterAuth.Password = config.Setting("get", "infrastructure", "Spec.Cluster.Auth.Password", "")
	return masterAuth
}
